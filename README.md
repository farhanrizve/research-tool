...
An AI-powered research assistant workspace with **56 specialized skills**, reusable LaTeX templates, data processing scripts, and automated research workflows — all designed to accelerate academic research from literature review to final publication.
...
- **🧠 56 AI Research Skills** — Auto-loaded by Copilot for literature review, paper analysis, LaTeX compilation, PDF/data extraction, academic writing, and more
...
# 4. Configure environment (optional — enables free Zen LLM)
copy .env.example .env
# Edit .env and set RESEARCH_ZEN_API_KEY for free LLM access

# 5. Verify everything is ready
python scripts/check_environment.py
```

> **Free LLM Access:** Copy `.env.example` to `.env` and set `RESEARCH_ZEN_API_KEY` to use Zen's free models (DeepSeek V4 Flash, MiMo V2.5, and more) — no API key purchase needed.
...
│
├── .agents/skills/           ← 56 AI skills (auto-loaded)
...
├── web/                      ← Web UI (optional)
└── src/research_tool/        ← Python package source
```
...
| **Environment**        | `environment-awareness` — detects system, runtimes, MCP tools               |
...
| [`.env.example`](.env.example)                         | Environment config template — copy to `.env` to start    |
